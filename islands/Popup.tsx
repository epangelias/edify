...
interface props {
	fields: Field[];
	Values: Signal<Deno.KvEntry<Record<string, unknown>>[]>;
	path: string[];
	state: { edifyConfig: EdifyConfig };
}

export function Popup({ fields: _fields, Values, path, state: { edifyConfig } }: props) {
	...
	function open() {
		...
		setAction(edifyConfig.basePath + '/edit/' + path.join('/') + '/' + ID);
